...
DSI_STATUS DSI_DumpRegisters(DISP_MODULE_ENUM module, int level)
{
	...
	if(level >= 0)
	{
		if(module == DISP_MODULE_DSI0/* || module == DISP_MODULE_DSIDUAL*/)
		{
			...
			DDPDUMP("DSI0 LFR En:%d, LFR MODE:%d, LFR TYPE:%d, LFR SKIP NUMBER:%d\n",
...
{	
	...
	for(i = DSI_MODULE_BEGIN(module);i <= DSI_MODULE_END(module);i++)
	{
		...
		// restore dsi irq inten
		DSI_OUTREG32(cmdq,&DSI_REG[i]->DSI_INTEN, AS_UINT32(&regs->DSI_INTEN));
		
		printk("DSI_RestoreRegisters DSI_INTEN 0x%08x\n",AS_UINT32(&regs->DSI_INTEN));
		printk("DSI_RestoreRegisters DSI_INTSTA 0x%08x\n",AS_UINT32(&DSI_REG[i]->DSI_INTSTA));
		
		//clear all dsi irq intsta bit
		DSI_OUTREG32(NULL,&DSI_REG[i]->DSI_INTSTA, 0);
...
#else
	mt_set_gpio_mode(GPIO28|0x80000000, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO28|0x80000000, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO28|0x80000000, value?GPIO_OUT_ONE:GPIO_OUT_ZERO);
#endif	
}
...
if(_dsi_is_video_mode(module))
{
		DSI_OUTREGBIT(NULL, DSI_INT_ENABLE_REG,DSI_REG[0]->DSI_INTEN,TE_RDY,0);
}
else
{
...
int ddp_dsi_power_on(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	//if (!s_isDsiPowerOn)
	{
		...
		DISPCHECK("DSI Why reset cause irq??? INTSTA=0x%08x",AS_UINT32(&DSI_REG[0]->DSI_INTSTA));
