...
void disp_set_pll(unsigned int freq)
{
    ...
    reg_va_con0 = (unsigned long) ioremap_nocache(REG_PA_VENC_PLL_CON0, sizeof(unsigned long));  
    reg_va_con1 = (unsigned long) ioremap_nocache(REG_PA_VENC_PLL_CON1, sizeof(unsigned long));  
    
    printk("disp_set_pll(%d), before set, con0=0x%x, con1=0x%x, 0x%lx, 0x%lx\n", 
        freq, 
        DISP_REG_GET(reg_va_con0), 
        DISP_REG_GET(reg_va_con1),
        reg_va_con0, 
        reg_va_con1);
    ...
    {
        printk("disp_set_pll, error, invalid freq=%d \n", freq);
    }

    printk("disp_set_pll(%d), after set, con0=0x%x, con1=0x%x \n", 
...
static void disp_debug_api(unsigned int enable, char *buf)
{
    ...
    {
        disp_set_pll(156);
        printk("DDP: disp_set_pll=156.\n");    
        sprintf(buf, "disp_set_pll=156.\n");               
    }
    else if(enable==47)
    {
        disp_set_pll(182);
        printk("DDP: disp_set_pll=182.\n");    
        sprintf(buf, "disp_set_pll=182.\n");               
    }
    else if(enable==48)
    {
        disp_set_pll(364);
        printk("DDP: disp_set_pll=364.\n");    
    ...
    else if(enable==49)
    {
        ...
        printk("DDP: gChangeRDMAThreshold=%d\n", gChangeRDMAThreshold);    
    ...
    else if(enable==51)
    {
        ...
        printk("DDP: gEnableUnderflowAEE=%d\n", gEnableUnderflowAEE);    
    ...
    {        
        ...
        printk("DDP: disp_set_pll_by_cmdq=156. estimate execute time=%d \n", time);    
    ...
    else if(enable==53)
    {
        ...
        printk("DDP: disp_set_pll_by_cmdq=182. estimate execute time=%d \n", time);    
    ...
    else if(enable==54)
    {
        ...
        printk("DDP: disp_set_pll_by_cmdq=364. estimate execute time=%d \n", time);    
