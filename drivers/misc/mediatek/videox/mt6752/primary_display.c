...
void primary_display_lfr_enable(int enable)
{
	LCM_PARAMS *lcm_param = disp_lcm_get_params(pgc->plcm);
	
	DISPCHECK("lcm param lfr_enable 0x%x,lfr_mode 0x%x,lfr_type 0x%x,lfr_skip_num 0x%x\n",lcm_param->dsi.lfr_enable,lcm_param->dsi.lfr_mode,lcm_param->dsi.lfr_type,lcm_param->dsi.lfr_skip_num);
	lcm_param->dsi.lfr_enable = enable;
		
	DSI_Set_LFR(DISP_MODULE_DSI0, NULL,lcm_param->dsi.lfr_mode,lcm_param->dsi.lfr_type,lcm_param->dsi.lfr_enable,lcm_param->dsi.lfr_skip_num);
...
int primary_display_save_power_for_idle(int enter, unsigned int need_primary_lock)
{
    ...
    if(disp_low_power_remove_ovl==1)
    {
        ...
        {            
             extern unsigned int disp_set_pll_by_cmdq(unsigned int freq, void * cmdq_handle);
...
static int _DL_switch_to_DC_fast(unsigned int lock)
{
	int ret = 0;
...
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	if(_need_lfr_check()==1)
	{		
		dpmgr_path_build_cmdq(pgc->dpmgr_handle, pgc->cmdq_handle_config,CMDQ_DSI_LFR_MODE);
	}
...
{
    static unsigned int isFirstError = 0;

    // first time error will dump to AEE folder
    if(isFirstError==0)
	{
	    isFirstError = 1;
	    gCMDQDump = 1;
	    DISPERR("cmdq timeout disp dump:%llu\n", engineFlag);
    	primary_display_diagnose();
...
static int _ovl_fence_release_callback(uint32_t userdata)
{
    ...
    {       
       ...
       {
           DISPMSG("disp ovl status:0x%x, 0x%x \n", ovl_status[0], ovl_status[1]);
...
//#endif	
	...
	if(gEnableIRQ==0)
	{
        DISP_CPU_REG_SET(DISP_REG_RDMA_INT_ENABLE, 0x1E);
	}
